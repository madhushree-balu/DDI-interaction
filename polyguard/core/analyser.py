...
class PolyGuardAnalyser:
    ...
    ----------
    loader : DataLoader
        A *loaded* DataLoader instance (call ``loader.load()`` before passing it in).
    """

    def __init__(self, loader: DataLoader) -> None:
        if not loader._loaded:
            raise ValueError("DataLoader must be loaded before passing to PolyGuardAnalyser.")
        self._loader = loader
        self._engine  = None   # lazy-import to avoid slowing startup
        self._xai     = None

    # ── Public API ─────────────────────────────────────────────────────────────

    def search_brands(self, prefix: str, limit: int = 10) -> BrandSearchResult:
        """Return brand names starting with *prefix*."""
    ...
    def get_ingredients(self, brand_name: str) -> IngredientsResult:
        """Return all active ingredients for a single brand name."""
    ...
    def analyse(
        ...
        """
        Run the full PolyGuard pipeline (Steps 1-7).

        Parameters
        ----------
        brand_names  : List of brand medication names.
        patient_data : Optional patient context dict with keys:
                         age (int), gender (str), conditions (list[str]),
                         lab_values (dict[str, float])
        explain      : Whether to generate XAI explanations.

        Returns
        -------
        AnalysisResult — a fully populated Pydantic model.
        """
        self._ensure_engine()

        # ── Steps 1 & 2: resolve brands → ingredients → interactions ─────────
        ...
        # ── Step 3: NLP severity scoring ─────────────────────────────────────
        engine    = self._engine
        base_scores = engine.calculate_interaction_score_robust(raw_interactions)

        # ── Step 4: organ system distribution ────────────────────────────────
        organ_analysis = engine.analyze_biological_impact(raw_interactions, base_scores)
    ...
    def _ensure_engine(self) -> None:
        """
        Lazy-import ``polyguard_engine_evidence_based`` so NLP model training
        only runs when ``.analyse()`` is first called (keeps startup fast).

        The project root is already on sys.path (injected by core/__init__.py),
        so the bare module name resolves correctly from anywhere.
        """
        if self._engine is not None:
            return
        import polyguard_engine_evidence_based as _eng  # type: ignore[import]
        self._engine = _eng
    ...
    def _build_result(
        ...
        summary = ReportSummary(
            overall_risk_level      = summary_raw.get("overall_risk_level", "MINIMAL"),
            risk_color              = summary_raw.get("risk_color", "⚪"),
            risk_icon               = summary_raw.get("risk_icon", ""),
            primary_action          = summary_raw.get("primary_action", ""),
            total_interaction_score = summary_raw.get("total_interaction_score", 0),
    ...
    def _map_xai(self, raw: Dict) -> XAIReport:
        ...
        return XAIReport(
            ...
            step6_cascade_attributions     = raw.get("step6_cascade_attributions", []),
